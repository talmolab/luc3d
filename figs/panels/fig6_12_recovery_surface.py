...
Fig 6c's own convention at stride 1 (figs/fig6_recovery.py).
...
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
from fig6_recovery import surface_from_hist  # noqa: E402
...
def main(surface=False):
    use()
    df = build()
    deposit(df, 6, "fig6c_recovery_surface.csv")
    ...
    if not surface:
        ...
        save(fig, 6, "c", "recovery_surface_3d")
