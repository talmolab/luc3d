...
(-0.028, 0/3). Those last two cells rest on 4 and 3 sessions; the panel prints n and
the win count under every tick so the reader can weigh them, and the negative cells
are NOT hidden. Pooled over all 74 sessions the difference is +0.075
...
def main():
    ...
    for i, r in df.iterrows():
        ...
        ax.annotate(f"{r['mean']:+.3f}", (i, r["mean"]), textcoords="offset points",
                    xytext=(9, 0), color=color, fontsize=6.5, va="center")

    ax.set_xticks(x)
    ax.set_xticklabels([f"{int(r.animals)}\nn = {int(r.n_sessions)}\n"
                        f"{int(r.wins)}/{int(r.n_sessions)}" for _, r in df.iterrows()])
    ax.set_xlabel("animals · sessions · wins", labelpad=2)
    ...
    footnote(ax, f"all {a['n_sessions']} sessions: {a['mean']:+.3f} "
            ...
            f"{a['wins']}/{a['n_sessions']}, sign test P = {a['sign_p']:.3f}")
