...
from fig6_variant_common import (FRESH_LS, arms, flags,  # noqa: E402
                                 pool_note, slug)
...
def main(variant=False, corrected=True, fresh_arm=False):
    ...
    if not variant:
        ...
        deposit(df, 6, f"{slug('fig6c_by_animals', variant, corrected, fresh_arm)}.csv")
    else:
            ...
            [{"arm": arm, "animals": a, "mean": blk["paired_vs_sleap"][str(a)]["mean"],
             ...
             for arm, blk, tk in (("LUC3D pre-#131 (manuscript panel)", ref,
                                 ...
                                 ("LUC3D + fresh anchor (shipped)", fresh,
                                  "fresh_anchor"))
             for a in counts]),
            6, "fig6c_by_animals_variant.csv")
    ...
    if variant:
        ...
        note += (
            ...
            f"\n{pool_note()}")
    footnote(ax, note)
    save(fig, 6, "c", slug("by_animals", variant, corrected, fresh_arm))
