...
from src.style import (MUTED, footnote, entity, deposit, panel, save,  # noqa: E402
                       text_legend, use)
from fig6_variant_common import (FRESH_LABEL, FRESH_NOTE, SHIPPED_LABEL,  # noqa: E402
...
def main(variant=False, corrected=True, fresh_arm=False):
    ...
    if not tcf:
        sys.exit("fig7e: the deposit has no slap2m.total_camera_frames -- re-run "
                 "figs/fig3_trackers.py (or figs/fig6_variant_tracker.py --slap2m, "
    ...
    deposit(df, 6,
            f"{slug('fig6d_error_decomposition', variant, corrected, fresh_arm)}.csv")
    ...
    if variant:
        ...
        note += (
            ...
            f"\n{pool_note()}")
    footnote(ax, note)
    save(fig, 6, "d", slug("decomposition", variant, corrected, fresh_arm))
