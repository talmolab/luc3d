...
Fig 7a -- within-view vs cross-view IDF1: which trackers hold identity ACROSS cameras.
    ...
    ############################################################################
    THE MANUSCRIPT PANEL `fig6a_within_vs_cross.pdf` STILL CONTAINS THREE DEFECTS.
...
* IT IS A PROPERTY OF THE SCORING CONVENTION, measured both ways on the same 50 sessions
  ...
  `figs/fig6_sleap_retracked.py` did not, pooling the track NAME so the same name in five
  cameras is one identity. `figs/fig6_sleap_scoped.py` scored both: SLEAP's corrected
...
`figs/fig6_sleap_retracked.py` scores the 250 `.slp` files DIRECTLY -- every track, no
fixed-width intermediate to overflow -- with the same IoU >= 0.5 matching and the same
motmetrics construction, and `figs/fig6_variant_tracker.py --fix-sleap` installs the result
...
(`fig6_sleap_retracked.py`) reported cross-view 0.0836, a 1.4x "correction", by pooling the
...
`figs/fig6_sleap_scoped.py` measured both conventions in one pass over the same files, gated
...
different panel. `figs/fig6-bench/xv_experimental.js` with method `{sync: true, stale: 20}`
...
* the variant writes a DIFFERENT SLUG, `fig6a_within_vs_cross_variant.pdf`, so it cannot
  overwrite the manuscript panel `fig6a_within_vs_cross.pdf` whatever letter it carries.
  It is lettered "a" (not "h", as an earlier version was; those `fig6h_*` artefacts have
...
`figs/fig6_variant_tracker.py --replace --fix-sleap --muppet-coverage`, which reads the
SLEAP measurement from `figs/out/fig7_sleap_scoped.json`
(`figs/fig6_sleap_scoped.py`: both pooling conventions, gated on reproducing
...
#: Built by `figs/fig6_fair_baselines.py` from the two re-runs:
...
#: `fig6_variant_tracker.py --fix-sleap`, which moves the old slot-index series there and
...
def main(variant=False, fair=True):
    ...
    # THE TABLE CARRIES THE SAME PANEL LETTER AS THE ARTWORK. This deposited `fig6h_*`
    # while `save()` below writes `fig6a_within_vs_cross_variant.*`, left over from when the
    ...
    deposit(df, 6, "fig6a_within_vs_cross_variant.csv" if variant
            else "fig6a_within_vs_cross.csv")
    ...
    if variant:
        ...
        note += (
            ...
            "benched in figs/fig6-bench/xv_experimental.js\n"
    ...
    if variant:
        ...
        # filename, so the manuscript panel fig6a_within_vs_cross.pdf can never be
        # overwritten by this branch.
        save(fig, 6, "a", "within_vs_cross_variant")
    else:
        save(fig, 6, "a", "within_vs_cross")
