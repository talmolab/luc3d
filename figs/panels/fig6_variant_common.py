...
prose about past events are the letters things had at the time).
    ...
    as supplementary `fig6s2_bedding` and still plots the pre-#131 arm for its own
    ...
    the `--variant` render draws both:
      ...
      2. SHIPPED -> FRESH ANCHOR. The experimental arm ({sync, stale 20} +
         distanceThreshold 25, `figs/fig6-bench/xv_experimental.js`, NOT in the
...
#: `figs/fig6_variant_tracker.py --config sync_stale20_dist25
    ...
    for k in (REF_KEY, FRESH_KEY, ARMS_KEY):
        if k not in t:
            sys.exit(f"fig7: {VARIANT_SRC} has no `{k}` -- it was built without "
                     ...
                     f"Rebuild it with:\n  $PY figs/fig6_variant_tracker.py --config "
...
def arms(variant, corrected=True, fresh=False):
    ...
    this file and in `fig6_variant_tracker.py` calling it "NOT in the app"
