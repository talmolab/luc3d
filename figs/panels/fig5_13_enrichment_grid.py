...
same colour in both (fig5_app.mjs PALETTE, derived in
...
def main():
    ...
    if not man_p.exists():
        sys.exit("missing figs/out/fig6-app.json — run `node figs/fig5_app.mjs`")
    ...
    if missing:
        sys.exit(f"missing figs/out/{missing} — run `node figs/fig5_app.mjs`")
