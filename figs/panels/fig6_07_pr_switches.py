...
the cross-view IDF1 survival curve, and ID switches per 100,000 camera-frames.
    ...
    THIS PANEL SHIPS AS THE SWEEP ROW OF FIGURE 11 (fig6_sync.py copies it in at
...
WHAT EACH PARAMETER SET IS, spelled out because "stale 10" means nothing on its own:
  ...
  no eviction                `pose/cross-view-tracker.js` with no staleness eviction --
                             ...
                             renamed (it is what `fig6_methods.py --all-sessions` wrote),
...
figs/fig6_pr_curve.py measured it). Version 3, THE ONE THAT SHIPS: a PR PLANE, one
...
`$PY figs/fig6_methods.py --all-sessions --configs shipped,sync_stale1_dist25,...`.
