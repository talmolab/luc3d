...
from src.style import (MUTED, entity, footnote, GREY, deposit, panel,  # noqa: E402
                       save, use)
from fig6_variant_common import (FRESH_LS, REF_LS, arms, flags,  # noqa: E402
                                 pool_note, slug)
...
def main(variant=False, corrected=True, fresh_arm=False):
    ...
    deposit(pd.DataFrame(rows), 6,
            f"{slug('fig6b_survival', variant, corrected, fresh_arm)}.csv")
    ...
    footnote(ax, f"one step per session; n = {luc['n_sessions']} SLAP-2M sessions\n"
             ...
             + ("" if not variant else
                ...
                f"\n{pool_note()}"))
    save(fig, 6, "b", slug("survival", variant, corrected, fresh_arm))
