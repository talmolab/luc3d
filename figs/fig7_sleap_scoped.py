...
WHY THIS EXISTS. `figs/fig7_sleap_retracked.py` fixed the real defect in Fig 7a's SLEAP
...
other is on disk beside it and the artwork names the convention.
    ...
    unscoped   hypothesis id = track name                (fig7_sleap_retracked.py's)
...
one line.
    ...
    $PY figs/fig7_sleap_scoped.py --sessions 20250829_124351   # one session
    $PY figs/fig7_sleap_scoped.py                              # all 50
...
#: the `--max_tracks 2` re-run (`figs/fig7_sleap_max2_retrack.py`) writes
...
def main():
    ...
    (OUT / a.out).write_text(json.dumps({
        "generated_by": "figs/fig7_sleap_scoped.py",
        ...
        "conventions": {
            ...
            "cross_unscoped": "the same pooling with the hypothesis id keyed on the track "
                              ...
                              "identity. This is what figs/fig7_sleap_retracked.py "
