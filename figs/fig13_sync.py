...
it fig13 for now so we dont undo any of our other work"). Fig 3 and Fig 4 are
...
(`fig11_sync.py`, "combine fig 7 and fig 10").

TWO VERSIONS WERE ORIGINALLY BUILT TO COMPARE (Eric, 2026-08-20: "make both
versions for me to see"): fig13 ("stack") and fig1301 ("side by side"). Eric
then said "figure 13 is close!" and, later, "forget about 1301 dont use it.
delete it." -- fig1301's LAYOUTS/TITLES entries, figures/fig1301/, and every
fig1301-specific line in this file are gone. Only fig13 is built here now.
...
colours reverted from the fig13-only AMBER/SKY substitute back to Fig 4's own
...
thicken the lines or something?"). `panels/fig13_03_quality.py` reuses Fig 3c's
...
the same colors as the id lines in 13c as well"). `panels/fig13_05_association.py`
...
blue/orange/green, writing an INTERMEDIATE `figures/fig13/_association_top.pdf`
that `build_stack`'s new `top_path` argument reads instead of the real Fig 3a
PDF -- see that script's own docstring.

Re-running a SOURCE panel script does NOT update fig13 -- re-run this after any
fig3 panel regenerates, and re-run the native fig13 panel scripts after
...
2026-08-25 rebuild -- see the geometry comment above CELL_W):
    ...
    .venv/bin/python figs/panels/fig13_00_hyp_illustration.py
    .venv/bin/python figs/panels/fig13_03_quality.py
    .venv/bin/python figs/panels/fig13_05_association.py
    .venv/bin/python figs/panels/fig13_06_idswitch.py
    .venv/bin/python figs/panels/fig13_07_sweep_split.py
    .venv/bin/python figs/panels/fig13_08_head_to_head.py
    .venv/bin/python figs/fig13_sync.py
    .venv/bin/python figs/assemble.py 13
    .venv/bin/python figs/_verify_fig13_type.py     # c/d type sizes agree?
...
this sync, then `_verify_fig13_type.py`, and copy back any constant it reports
...
#: panels/fig13_00_hyp_illustration.py's own content_crop. Recomputed here (not
...
#: fig13 ("stack"): scale applied uniformly to BOTH a and b so that the a/b
...
#: NOMINAL cell height for the native panel scripts (fig13_03/_07/_08). sync()
...
def build_stack(dst_fig: int, dst_letter: str, top: tuple, bottom: tuple,
    ...
    deposited panel (e.g. `panels/fig13_05_association.py`'s
    figures/fig13/_association_top.pdf). `top` is still consulted for the dst
    filename's slug.
    """
    ...
    # assembled fig13.pdf, not just this composite's own text: PyMuPDF's
...
def build_column(dst_fig: int, dst_letter: str, dst_slug: str, items: list,
    ...
    row's panel beside several rows of others, so Eric's fig13 rectangle --
...
def sync() -> None:
    hyp = FIGURES / "fig13" / "fig13c_hyp_illustration.pdf"
    quality = FIGURES / "fig13" / "fig13e_quality.pdf"
    assoc_top = FIGURES / "fig13" / "_association_top.pdf"
    if not hyp.exists():
        print(f"  MISSING {hyp.relative_to(FIGS)} -- run "
              f"panels/fig13_00_hyp_illustration.py first")
        return
    if not quality.exists():
        print(f"  MISSING {quality.relative_to(FIGS)} -- run "
              f"panels/fig13_03_quality.py first")
        return
    if not assoc_top.exists():
        print(f"  MISSING {assoc_top.relative_to(FIGS)} -- run "
              f"panels/fig13_05_association.py first")
        return

    # ---- fig13: a/b stacked, c spans both rows beside them ----
    # top_path: "a" is a RECOLOURED copy of Fig 3a (panels/fig13_05_association.py,
    # matching 13c's animal colours), not Fig 3a's own PDF.
    # bottom: fig13's OWN half-span cost_model twin (fig3's b re-spanned to a
    ...
    build_stack(13, "a", (3, "a", "association"), (13, "b", "cost_model"), AB_STACK_SCALE,
    ...
    # geometry comment above CELL_W). All four cells are NATIVE fig13 panels
    # at ~CELL_W x CELL_H (e by panels/fig13_03_quality.py, f by
    # panels/fig13_08_head_to_head.py, g/h by panels/fig13_07_sweep_split.py);
    ...
    idsw = FIGURES / "fig13" / "fig13d_idswitch.pdf"
    cells = [("e", "quality"), ("f", "head_to_head"),
             ("g", "sweep_switches"), ("h", "sweep_idf1")]
    missing = [p for p in [idsw] + [FIGURES / "fig13" / f"fig13{l}_{s}.pdf"
    ...
    for letter, slug in cells:
        rescale_pdf(FIGURES / "fig13" / f"fig13{letter}_{slug}.pdf", CELL_W, cell_h)
    col_h = build_column(13, "e", "quality_col",
                         [(13, "e", "quality", "e"),
                          (13, "f", "head_to_head", "f")], 1.0)
    build_column(13, "g", "sweep_col",
                 [(13, "g", "sweep_switches", "g"),
                  (13, "h", "sweep_idf1", "h")], 1.0)
