...
`stale: 10` + `distanceThreshold 25` -- exists ONLY in `figs/fig6-bench/xv_experimental.js`,
...
at once and a reader has to be able to tell which is which:
  ...
  slap2m_fresh_anchor         the EXPERIMENTAL arm (`figs/fig6-bench/xv_experimental.js`,
                              {sync, stale 20} + distanceThreshold 25). NOT in the app.

WHAT IS RE-SCORED AND WHAT IS NOT. `figs/fig6_slap2m_rescore.py` calls luc3d-bench's own
...
`figs/fig6_sleap_retracked.py` scores the 250 `.slp` files DIRECTLY, every track, so the
...
`figs/fig6_sleap_retracked.py` pools the five cameras with the track NAME as the hypothesis
...
`--fix-sleap` reads `fig7_sleap_scoped.json` (`figs/fig6_sleap_scoped.py`, which measures
...
compare different sessions.
    ...
    $PY figs/fig6_variant_tracker.py [--config sync_stale10_dist25] [--fix-sleap]
...
#: `--fix-sleap`. The deposit written by `figs/fig6_sleap_scoped.py` -- the CAMERA-SCOPED
...
#: `--slap2m`. Where `figs/fig6_slap2m_rescore.py` leaves its drop-in CSVs and its
...
def install_slap2m(out, base):
    ...
    if not gate_p.exists():
        raise SystemExit(
            ...
            f"figs/fig6_slap2m_rescore.py --gate --adopt-shipped --arms fresh")
    ...
    out["slap2m_luc3d_is_substituted"] = (
        ...
        "(session, camera) by figs/fig6_slap2m_rescore.py through luc3d-bench's own "
        ...
        f"({{sync, stale 20}} + distanceThreshold 25, figs/fig6-bench/xv_experimental.js, "
    ...
    out.setdefault("caveats", []).extend([
        ...
        f"`{SLAP2M_FRESH_KEY}` is figs/fig6-bench/xv_experimental.js with method "
        ...
        f"outputs through figs/fig6_slap2m_rescore.py reproduces _eval_baseline.csv's luc3d "
...
def fix_sleap(out, base, luc_by_session):
    ...
    if not src.exists():
        raise SystemExit(
            ...
            f"  /root/vast/eric/luc3d-bench/liezl_env/bin/python figs/fig6_sleap_scoped.py\n"
    ...
    out.setdefault("caveats", []).extend([
        ...
        f"The first correction pass (figs/fig6_sleap_retracked.py) pooled UNSCOPED instead, "
...
def main():
    ...
    out["generated_by"] = ("figs/fig6_variant_tracker.py -- fig3_trackers.json plus one "
                           "EXPERIMENTAL tracker series"
                           + (" and a CORRECTED SLEAP series (--fix-sleap)"
                              if a.fix_sleap else "")
                           + "; NOT the manuscript deposit")
    out.setdefault("caveats", []).extend([
        f"`{a.label}` is figs/fig6-bench/xv_experimental.js with method "
    ...
    if a.replace:
        out["luc3d_is_experimental"] = (
            ...
            f"figs/fig6-bench/xv_experimental.js, cell {a.config}. The shipped "
