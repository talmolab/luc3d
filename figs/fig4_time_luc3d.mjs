 ...
 *
 * WHY THIS EXISTS. `fig4_measure.mjs` already deposits `methods.{dlt,ba}
 ...
 * first `--n` keypoints, in the SAME scope fig4_measure.mjs uses, immediately
 * before/after the anipose timing, so `fig4_anipose.py` can check the two agree
 * and refuse to plot if they do not.
 *
 * SCOPE, and it matters. fig4_measure.mjs times ONLY the solve:
 *
 ...
 * `fig4_anipose.py` reports anipose both ways.
 *
 * Reads figs/out/fig4_input.json + .bin (bin-v1 only). Writes JSON on stdout.
 *
 * Env: POSE_DIR=  (default ../lucid-bundle-adj/pose, as fig4_measure.mjs)
 * Usage: node figs/fig4_time_luc3d.mjs --n 200000 --repeats 3
 */
...
for (let r = 0; r < REPEATS; r++) {
    ...
    // KEEP THE RESULT IN A LOCAL, exactly as fig4_measure.mjs does. Storing each
    // solve into an `Array(n)` instead measured 13 us/kp against fig4_measure.mjs's
    ...
    // refinement -- which is what fig4_measure.mjs's `ba` number is.
...
process.stdout.write(JSON.stringify({
    ...
    note: ('best-of-repeats, single-threaded, JIT warmed on a 20k prefix. Scope '
           + 'matches fig4_measure.mjs: undistortion is timed SEPARATELY and is '
