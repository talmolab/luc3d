#!/usr/bin/env node
/**
 * fig6_bench.mjs — Fig 8 METHOD-SEARCH driver. A fork of figs/fig3-bench/fig3_bench.mjs
 * (itself a fork of scripts/bench/bench_crossview.mjs) that differs in exactly two ways:
 *
 *   1. It registers `figs/fig6-bench/hooks6.mjs` instead of `scripts/bench/hooks.mjs`.
 *      That hook is the base hook PLUS one redirect: `pose/cross-view-tracker.js` is
 *      served from `figs/fig6-bench/xv_experimental.js`. pose/tracker.js,
 ...
 *   node fig6_bench.mjs --session-idx N --num-animals N \
...
const HOOKS_URL = pathToFileURL(path.resolve(HERE, 'hooks6.mjs')).href;
...
async function main() {
    ...
    process.stderr.write(
        `[fig6_bench] session-idx=${opts.sessionIdx} frames=${usedFrames.length} dets=${totalDets} ` +
    ...
    process.stderr.write(
        `[fig6_bench] wrote ${frames.length} frames w/ assignments, ${identities.length} identities, ` +
