...
y ... how many points are recovered by the number of cameras included").
    ...
    The matching is fig6_detections.py's own `match_frame_wise` (mean-keypoint-
...
ride along from the master sheet.
    ...
    $PY figs/fig6_recovery.py --pilot          # one session
    $PY figs/fig6_recovery.py --workers 12     # all 74
...
def session_histogram(row, sidx):
    ...
    Loading and matching mirror fig6_detections.measure_session exactly (same
...
def main():
    ...
    (OUT / "fig6_recovery.json").write_text(json.dumps({
        "generated_by": "figs/fig6_recovery.py",
