...
Figure 1A shows one such recording setup — the SLAP-2M cage, its eight cameras and two
animals, rendered from a single session's own calibration and tracked 3D poses.
...
(Figure 1B). The three stages it contributes are cross-view re-identification,
...
one left over is a duplicate detection of an already-matched animal in a view the
panel does not show, which one-to-one assignment correctly refuses. Figure 1D
