...
`figs/fig6_variant_tracker.py` asks the shipped tracker to reproduce the deposit exactly.
...
writes the verdict to `<out>/gate.json`; `figs/fig6_variant_tracker.py --slap2m` refuses to
...
`figs/data/fig6/slap2m_luc3d_shipped_percam_ITEM3.csv` (header byte-identical to
...
it, and `--check-shipped` re-derives it and DIFFS against the deposited one.
    ...
    $PY figs/fig6_slap2m_rescore.py --gate --arms fresh --workers 8
...
def gate(percam_csv):
    ...
    v = {"passed": mx == 0.0, "n_rows": len(ref), "n_metric_cols": len(METRIC_COLS),
         ...
         "claim": ("evaluate.eval_camera(no_sleap=True), driven by "
                   "figs/fig6_slap2m_rescore.py over outputs/luc3d_results/, reproduces "
                   ...
                   "reason."),
         "generated_by": "figs/fig6_slap2m_rescore.py --gate",
