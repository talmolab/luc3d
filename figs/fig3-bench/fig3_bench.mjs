#!/usr/bin/env node
/**
 * fig3_bench.mjs — Fig 3 benchmarking driver. A fork of scripts/bench/bench_crossview.mjs
 * that adds --params support (JSON file: {thresholds:{...}, nodeWeights:{...}}) so the
 * corr2d x corr3d sweep (Task 1) can override CrossViewTracker hyperparameters per run.
 * Kept in figs/ per the handoff's "no app-source edits" rule — scripts/bench/bench_crossview.mjs
 * itself is untouched; this is a new file that imports the same real, unmodified
 * pose/tracker.js + pose/cross-view-tracker.js + pose/triangulation.js + pose/pose-data.js
 * (UI stubbed via the existing scripts/bench/hooks.mjs, read not modified) and drives
 * runCrossViewTracker() exactly as trackAll() does.
 *
 * Output JSON is IDENTICAL in shape to bench_crossview.mjs's, so evaluate.py / the
 * cross_view_metric.py-style scorers work unchanged.
 *
 * CLI:
 *   node fig3_bench.mjs --session-idx N --num-animals N \
...
async function main() {
    ...
    globalThis.__BENCH = { nodeWeights, thresholds: params.thresholds || {} };
    ...
    process.stderr.write(
        `[fig3_bench] session-idx=${opts.sessionIdx} frames=${usedFrames.length} dets=${totalDets} ` +
    ...
    process.stderr.write(
        `[fig3_bench] wrote ${frames.length} frames w/ assignments, ${identities.length} identities, ` +
