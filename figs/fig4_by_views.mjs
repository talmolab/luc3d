 ...
 *
 * The mm arm was what panel b plotted until `fig4_move_geometry.mjs` measured why it
 ...
 * raw detection, matching `fig4_measure.mjs`'s `heldout_by_views`, and cross-checks
 ...
 * half-open range of blocks; `node figs/fig4_by_views.mjs --merge a.json b.json ...`
 ...
 * Usage: node figs/fig4_by_views.mjs
 *        BLOCKS=0:10 OUT_JSON=out/shard0.json node figs/fig4_by_views.mjs
 *        node figs/fig4_by_views.mjs --merge out/shard*.json
 */
    ...
    for (let kk = lo; kk < hi; kk += STRIDE) {
        ...
        for (let k = 2; k <= C; k++) {
            for (const m of SUBSETS[k]) {
                ...
                // exactly as fig4_measure.mjs's `heldout_by_views` does. `raw`, not
...
const out = summarize(perSession, C, {
    ...
    caveat: ('err3d_mm_* is 3D distance to the proofread reference and is DEPOSITED '
        + 'BUT NOT PLOTTED: fig4_move_geometry.mjs shows the refinement moves a '
