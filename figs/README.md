...
| `assemble.py` | Places the panel PDFs into `figures/figN/figN.pdf` as **vector** content (`show_pdf_page`), with panel letters in Helvetica-Bold. Layout is **by rows**: only each panel's width is given and its height follows from its own aspect, so panels cannot overlap. Warns past 200 mm. |
