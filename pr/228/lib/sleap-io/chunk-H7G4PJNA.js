...
var Instance3D = class {
  ...
  get nVisible() {
    ...
    // LUCID local patch (luc3d #189): `points` may be a FLAT Float64Array of
    // 3*nNodes (see reconstructColumnarFrameGroups) as well as boxed rows.
    if (ArrayBuffer.isView(this.points)) {
      let n = 0;
      for (let i = 0; i + 2 < this.points.length; i += 3) {
        if (!Number.isNaN(this.points[i]) && !Number.isNaN(this.points[i + 1]) &&
            !Number.isNaN(this.points[i + 2])) n++;
      }
      return n;
    }
    ...
    for (let g = igStart; g < igEnd; g++) {
      ...
      if (pts3dStart >= 0) {
        ...
        if (src) {
          ...
          // LUCID local patch (luc3d #189): read-side mirror of the #185 write
          // fix. Upstream pushed one boxed `[x,y,z]` Array per keypoint out of
          // `flat`, which is ALREADY a Float64Array — on the real 180,210-frame
          // project that is 7,976,985 boxed rows (~410 MB) allocated in V8's
          // pointer-compressed heap, the resource a Chrome renderer hard-caps
          // near 4 GB, and it is what made reopening a large saved project
          // spend 8+ minutes in a GC death spiral at the ceiling. Emitting one
          // compacted Float64Array per instance group instead puts the
          // coordinates in a backing store OUTSIDE that cap.
          //
          // Compacted (stride 3) rather than a subarray view of `flat`, because
          // the predicted table is stride 4 (x,y,z,score) and a view would also
          // pin the whole multi-hundred-MB matrix alive for as long as any one
          // group survives.
          const { flat, ncols } = src;
          const nPts3d = pts3dEnd - pts3dStart;
          const points = new Float64Array(nPts3d * 3);
          const pointScores = predicted ? new Float64Array(nPts3d) : [];
          for (let r = pts3dStart; r < pts3dEnd; r++) {
            const base = r * ncols;
            const out = (r - pts3dStart) * 3;
            points[out] = flat[base];
            points[out + 1] = flat[base + 1];
            points[out + 2] = flat[base + 2];
            if (predicted) pointScores[r - pts3dStart] = flat[base + 3];
          }
