...
export class Group extends HasAttrs {
    ...
    *items() {
    ...
    /**
     * Create a dataset with a compound (structured) datatype.
     * Each field has a name, HDF5 type class, byte size, and signedness.
     */
    create_compound_dataset(args) {
        const { name, data, fieldNames, fieldTypes, fieldSizes, fieldSigns, nrows, rowSize } = args;
        const dataBytes = new Uint8Array(data);
        let data_ptr = check_malloc(dataBytes.byteLength);
        try {
            Module.HEAPU8.set(dataBytes, data_ptr);
            Module.create_compound_dataset(this.file_id, this.path + "/" + name, fieldNames, fieldTypes, fieldSizes, fieldSigns, BigInt(data_ptr), nrows, rowSize);
        }
        finally {
            Module._free(data_ptr);
        }
