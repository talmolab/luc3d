      ...
      for (const group of frameGroup.instanceGroups) {
        ...
        if (points3d) {
          ...
          if (isPred) {
            ...
            // LUCID local patch (luc3d #185): straight into the typed sink.
            points3d.forEach((row, i) => {
              predPts3dSink.pushCoerced(row, 3, scores[i]);
            });
            ...
            // LUCID local patch (luc3d #185): straight into the typed sink.
            for (const row of points3d) pts3dSink.pushCoerced(row, 3);
