...
var MediaBunnyVideoBackend = class _MediaBunnyVideoBackend {
  ...
  async initialize() {
    ...
    // LUCID local patch (#115): EncodedPacketSink.packets() yields packets in
    // DECODE order, not presentation order (mediabunny's own docs: "packets
    // in decode order"; each packet's .timestamp is its PTS, but the
    // iteration order is not sorted by it). For any B-frame-encoded video
    // (routine for real camera recordings), decode order != presentation
    // order, so pushing timestamps in iteration order made _frameTimes[i]
    // NOT the i-th frame in display order — decodeSingleFrame(i) then looked
    // up the WRONG timestamp for any i displaced by B-frame reordering,
    // deterministically returning the wrong frame's pixel content for a
    // CORRECT frame index every time (not a race — reproduces on a single,
    // deliberate, non-racing step). Sorting ascending by timestamp here makes
    // _frameTimes[i] the correct i-th frame in presentation/playback order.
    this._frameTimes.sort(function (a, b) { return a - b; });
  ...
  async decodeSingleFrame(frameIndex) {
    ...
    videoFrame.close();
    if (typeof sample.close === "function") sample.close(); // LUCID local patch (#115): close the VideoSample to avoid a WebCodecs resource leak
    ...
    this.decodingPromise = (async () => {
      try {
        ...
        for await (const sample of sink.samples(startTime, endTime)) {
          ...
          if (typeof sample.close === "function") sample.close(); // LUCID local patch (#115): close every VideoSample to avoid a WebCodecs resource leak
        }
      } finally {
...
var LazyDataStore = class _LazyDataStore {
  ...
  materializeFrame(frameIdx) {
    ...
    for (let instIdx = instStart; instIdx < instEnd; instIdx++) {
      ...
      if (catLink && this.categoryCatalog && catLink[0] >= 0 && catLink[0] < this.categoryCatalog.length) {
...
async function readFromStreamingFile(file, url, filenameHint, openVideos = false, onProgress, rawSessions = false, lazy = false, lazyVideoMetadata = false) {
  ...
  if (labeledFrames || lazyStore) {
    const { links, embs, catLinks, catEmbs, categoryCatalog } = await readStreamingInstanceIdentity(file);
...
async function readStreamingInstanceIdentity(file) {
  const rootKeys = file.keys();
  const readOwnerInstanceLinks = async (group, idxField, scoreField) => {
    const out = /* @__PURE__ */ new Map();
    try {
  ...
  const readOwnerInstanceEmbs = async (vecName, otName, oidName) => {
    const out = /* @__PURE__ */ new Map();
    try {
  ...
  let categoryCatalog = [];
  try {
    if (rootKeys.includes("categories") && (await file.getKeys("categories")).includes("name")) {
...
function writeLazyFramesAndInstances(file, store) {
  ...
  writeLazyMatrixDataset(
    ...
    "<d"
    // LUCID local patch (sleap-io.js#231): h5wasm parses "<f8" as FLOAT32, so
    // point_id_start/end quantize to even ints beyond 2^24 point rows and
    // silently corrupt every larger file's spans. "<d" is h5wasm's float64.
  );
  writeLazyMatrixDataset(
    file,
    "points",
...
async function openSlpWriter(header) {
  ...
  try {
    ...
    // LUCID local patch (sleap-io.js#231): "<d" (h5wasm float64), NOT "<f8"
    // (h5wasm parses it as float32 → point ids corrupt beyond 2^24 rows).
...
function writeEmbeddingTriple(file, c, names, get) {
  const vectors = [];
  const ownerTypes = [];
  const ownerIds = [];
  let dim = -1;
...
function collectCategoryCatalog(c) {
  const seen = /* @__PURE__ */ new Set();
  const catalog = [];
...
function writeCategoryLinks(file, c) {
  const catalog = collectCategoryCatalog(c);
  if (!catalog.length) return;
  const catToIdx = new Map(catalog.map((n, i) => [n, i]));
  const rows = [];
  for (const [ownerType, list] of ownerCollections(c)) {
    list.forEach((det, ownerId) => {
      const cat = det.category;
...
function anyDetection(labels, pred) {
  const any = (list) => list.some(pred);
...
function hasPerDetectionCategory(labels) {
  return anyDetection(
    labels,
    (d) => d.category != null && d.category !== "" || d.categoryEmbedding != null
  );
}
...
// LUCID local patch (luc3d #185): pre-sized typed-array row accumulator for the
// `/session_data` 3D-point tables. `writeSessions` used to push one boxed
// `Array(3|4)` per 3D keypoint into a plain JS array (`coerce3dRow` above) and
// hold them ALL live until `createGzipFloatMatrix` flattened them into a
// `Float64Array` moments later. On the real 180,210-frame x 5-camera project
// that is 531,799 instance groups x 15 nodes = 7,976,985 small arrays: at ~48 B
// per 3-double JSArray plus the outer element pointers, an estimated ~400 MB
// sitting in V8's pointer-compressed heap, which a Chrome renderer hard-caps
// near 4 GB (measured `jsHeapSizeLimit` 3.76 GB; `--max-old-space-size` does
// NOT raise it). A typed array's backing store is allocated OUTSIDE that cap,
// so accumulating straight into one moves that cost off the scarce resource —
// the whole table becomes a single 7,976,985 x 3 x 8 B = ~191 MB buffer — and
// removes the transient copy that flattening used to need. Sized by an exact
// counting pre-pass, so there is never a reallocation.
//
// Measured end-to-end on that project (same harness, pre-save baselines within
// 3 MB): unpatched -> renderer OOM crash 13 s into Save As, inside this
// function; patched -> 1,404,804,682 bytes written in 49.5 s.
function Float64RowSink(nrows, ncols) {
  this.ncols = ncols;
  this.rows = 0;
  this.data = new Float64Array(Math.max(1, nrows) * ncols);
}
// Writes `width` coerced values from `row` (null row => all-NaN, matching
// `coerce3dRow`), plus a trailing `extra` column when ncols > width.
Float64RowSink.prototype.pushCoerced = function (row, width, extra) {
  const d = this.data;
  const off = this.rows * this.ncols;
  // Out-of-range typed-array writes are SILENTLY DISCARDED, so an undercount in
  // the sizing pre-pass would corrupt the saved 3D points with no error at all.
  // Fail loudly instead.
  if (off + this.ncols > d.length) {
    throw new Error(
      `Float64RowSink overflow: row ${this.rows} exceeds capacity ${d.length / this.ncols} ` +
      `(ncols=${this.ncols}) — the /session_data 3D-point counting pre-pass no longer ` +
      `matches the write loop (LUCID local patch, luc3d #185).`
    );
  }
  if (row == null) {
...
// LUCID local patch (luc3d #189): push one row straight out of a caller's FLAT
// Float64Array (`src[srcOff .. srcOff+width]`) with no boxed row in between.
// LUCID's `InstanceGroup.points3d` is now itself a flat Float64Array(3N), so the
// whole save path can stay typed end to end.
Float64RowSink.prototype.pushFlat = function (src, srcOff, width, extra) {
  const d = this.data;
  const off = this.rows * this.ncols;
  if (off + this.ncols > d.length) {
    throw new Error(
...
// LUCID local patch (luc3d #189): number of 3D KEYPOINTS in a points value that
// may be either boxed rows (`[[x,y,z], ...]`, length = nNodes) or LUCID's flat
// Float64Array (length = 3 * nNodes). Getting this wrong silently mis-sizes the
// sink, so it is shared by the counting pre-pass and the write loop.
function lucidCount3dRows(p) {
  if (p == null) return 0;
  return ArrayBuffer.isView(p) ? (p.length / 3) | 0 : p.length;
}
// LUCID local patch (luc3d #190): growable flat row accumulator for the
// `/session_data` STRUCT tables (frame_groups, instance_groups,
// instance_group_members).
//
// Upstream pushes one boxed `Array` per row and holds them all live until
// `createMatrixDataset` flattens them. On the real 180,210-frame x 5-camera
// project that is 531,799 frame-group rows + 531,799 instance-group rows +
// 2,627,453 member rows. Measured (tests/e2e/_bench-writesessions.mjs): the
// writer's peak heap grows to 1,190 MB at 400,000 groups / 2,000,000 members,
// i.e. roughly 1 GB of boxed temporaries at this project's scale — landing on
// top of an already ~2.4 GB baseline and pushing the renderer past its hard
// ~4 GB pointer-compressed heap cap. The result is not a crash but a GC death
// spiral: a save that ran over 30 minutes without finishing.
//
// This is the same fix #185 applied to the 3D-point tables, extended to the
// struct tables. Time was never the problem — that bench shows writeSessions is
// LINEAR (25.2 -> 9.4 us/group as fixed costs amortize) — so this targets the
// allocation only, and the flushed bytes are unchanged.
//
// Growth doubles rather than pre-counting: the member count is only knowable by
// running `instanceGroupMemberRows`, and walking that twice would cost more than
// the at-most-2x transient a doubling sink holds.
function Float64GrowSink(ncols) {
  this.ncols = ncols;
  this.rows = 0;
  this.data = new Float64Array(Math.max(1, ncols) * 1024);
}
Float64GrowSink.prototype._ensure = function (extraRows) {
  const need = (this.rows + extraRows) * this.ncols;
  if (need <= this.data.length) return;
  let cap = this.data.length;
  while (cap < need) cap *= 2;
  const next = new Float64Array(cap);
  next.set(this.data.subarray(0, this.rows * this.ncols));
  this.data = next;
};
/** Append one row from a boxed array (values coerced, missing -> NaN). */
Float64GrowSink.prototype.pushRow = function (row) {
  this._ensure(1);
  const off = this.rows * this.ncols;
  for (let i = 0; i < this.ncols; i++) {
    const v = row[i];
    this.data[off + i] = v == null ? Number.NaN : Number(v);
  }
  this.rows++;
};
/** Append one row from up to 8 positional values — no boxed row at all. */
Float64GrowSink.prototype.push = function (a, b, c, d, e, f, g, h) {
  this._ensure(1);
  const o = this.rows * this.ncols, n = this.ncols, D = this.data;
...
function createMatrixDatasetTyped(file, name, sink, fieldNames, dtype) {
  ...
  let data = sink.data.length === needed ? sink.data : sink.data.subarray(0, needed);
  if (dtype.includes("i") && dtype.includes("4")) data = Int32Array.from(data);
...
// LUCID local patch (luc3d #185): typed-array counterpart of
// createGzipFloatMatrix — identical dataset parameters, but takes the already
// flat Float64Array instead of re-flattening an array of boxed rows.
function createGzipFloatMatrixTyped(file, name, sink, ncols) {
  const n = sink.rows;
  const needed = n * ncols;
  const data = sink.data.length === needed ? sink.data : sink.data.subarray(0, needed);
  file.create_dataset({
    name,
    data,
    shape: [n, ncols],
...
function instanceGroupMemberRowsInto(group, session, frameGroup, labeledFrameIndex, sink) {
  ...
  for (const camera of seen) {
    ...
    if (!pair && instRefs?.has(camera)) pair = instRefs.get(camera);
    if (pair) sink.push(camIdx, pair[0], pair[1]);
  }
}
  ...
  for (const session of sessions) {
    ...
    for (const frameGroup of session.frameGroups.values()) {
      ...
      for (const group of frameGroup.instanceGroups) {
        ...
        // synchronous loop, where V8 cannot keep up with the churn.
        instanceGroupMemberRowsInto(
          group,
          session,
          frameGroup,
          labeledFrameIndex,
        ...
        if (points3d) {
          ...
          // LUCID local patch (luc3d #185/#189): straight into the typed sink,
          // and flat-to-flat when the caller's points are already a
          // Float64Array (LUCID's `InstanceGroup.points3d`) — no boxed row is
          // ever materialized on the save path.
          const isFlat3d = ArrayBuffer.isView(points3d);
...
function writeLabeledFrames(file, labels) {
  ...
  createMatrixDataset(
    ...
    "<d"
    // LUCID local patch (sleap-io.js#231): h5wasm float64 is "<d"; "<f8" is
    // parsed as float32, corrupting point ids beyond 2^24 point rows.
  );
  createMatrixDataset(
    file,
    "points",
...
function readLinksTable(file, path, idxField, scoreField, emscripten) {
  const result = /* @__PURE__ */ new Map();
...
var readIdentityLinks = (file, emscripten) => readLinksTable(
  file,
  "identity/links",
  "identity_idx",
  "identity_score",
...
var readCategoryLinks = (file, emscripten) => readLinksTable(
  file,
  "categories/links",
  "category_idx",
  "category_score",
...
function readCategoryCatalog(file) {
  const ds = file.get("categories/name");
  if (!ds) return [];
  return (ds.value ?? []).map(decodeStr);
}
function readEmbeddingTriple(file, vecName, otName, oidName) {
  const result = /* @__PURE__ */ new Map();
...
var readEmbeddings = (file) => readEmbeddingTriple(file, "vectors", "owner_type", "owner_id");
var readCategoryEmbeddings = (file) => readEmbeddingTriple(
  file,
  "category_vectors",
  "category_owner_type",
  "category_owner_id"
...
function attachCategoryToOwners(owners, catalog, links, embs) {
  if (links) {
    for (const [ownerId, [idx, score]] of links) {
      const det = owners[ownerId];
...
function readPerDetectionMaps(file, emscripten) {
  return {
...
function attachOwnerType(maps, ownerType, owners, identities) {
