...
function writeMetadata(file, labels) {
  ...
  metadataGroup.create_attribute("json", jsonStr, null, `S${jsonByteLen}`);
}
function serializeSkeletons(skeletons) {
  ...
  const serialized = skeletons.map((skeleton) => {
    const links = [];
    let edgeInsertIdx = 0;
    let seenEdgeType1 = false;
    let seenEdgeType2 = false;
    for (const edge of skeleton.edges) {
      const source = nodeIndex.get(edge.source.name) ?? 0;
      const target = nodeIndex.get(edge.destination.name) ?? 0;
      const type = !seenEdgeType1 ? { "py/reduce": [{ "py/type": "sleap.skeleton.EdgeType" }, { "py/tuple": [1] }] } : { "py/id": 1 };
      links.push({ edge_insert_idx: edgeInsertIdx++, key: 0, source, target, type });
      seenEdgeType1 = true;
    }
    for (const [left, right] of skeleton.symmetryNames) {
      const source = nodeIndex.get(left) ?? 0;
      const target = nodeIndex.get(right) ?? 0;
      const type = !seenEdgeType2 ? { "py/reduce": [{ "py/type": "sleap.skeleton.EdgeType" }, { "py/tuple": [2] }] } : { "py/id": 2 };
      links.push({ edge_insert_idx: edgeInsertIdx++, key: 0, source, target, type });
      seenEdgeType2 = true;
    }
    const skelNodes = skeleton.nodeNames.map((name) => ({
      id: nodeIndex.get(name) ?? 0
    }));
    return {
      directed: true,
      graph: { name: skeleton.name ?? "Skeleton", num_edges_inserted: edgeInsertIdx },
      links,
      multigraph: true,
      nodes: skelNodes
    };
...
function writeLabeledFrames(file, labels) {
  ...
  createCompoundDataset(file, "frames", frames, [
    { name: "frame_id", type: 0, size: 8, signed: false },
    // u8
    { name: "video", type: 0, size: 4, signed: false },
    // u4
    { name: "frame_idx", type: 0, size: 8, signed: false },
    // u8
    { name: "instance_id_start", type: 0, size: 8, signed: false },
    // u8
    { name: "instance_id_end", type: 0, size: 8, signed: false }
    // u8
  ]);
  createCompoundDataset(file, "instances", instances, [
    { name: "instance_id", type: 0, size: 8, signed: true },
    // i8
    { name: "instance_type", type: 0, size: 1, signed: false },
    // u1
    { name: "frame_id", type: 0, size: 8, signed: false },
    // u8
    { name: "skeleton", type: 0, size: 4, signed: false },
    // u4
    { name: "track", type: 0, size: 4, signed: true },
    // i4
    { name: "from_predicted", type: 0, size: 8, signed: true },
    // i8
    { name: "score", type: 1, size: 4, signed: false },
    // f4
    { name: "point_id_start", type: 0, size: 8, signed: false },
    // u8
    { name: "point_id_end", type: 0, size: 8, signed: false },
    // u8
    { name: "tracking_score", type: 1, size: 4, signed: false }
    // f4
  ]);
  createCompoundDataset(file, "points", points, [
    { name: "x", type: 1, size: 8, signed: false },
    // f8
    { name: "y", type: 1, size: 8, signed: false },
    // f8
    { name: "visible", type: 0, size: 1, signed: false },
    // u1 (bool)
    { name: "complete", type: 0, size: 1, signed: false }
    // u1 (bool)
  ]);
  createCompoundDataset(file, "pred_points", predPoints, [
    { name: "x", type: 1, size: 8, signed: false },
    // f8
    { name: "y", type: 1, size: 8, signed: false },
    // f8
    ...
    { name: "complete", type: 0, size: 1, signed: false },
    // u1 (bool)
    { name: "score", type: 1, size: 8, signed: false }
    // f8
  ]);
}
...
function createCompoundDataset(file, name, rows, fields) {
  ...
  const rowSize = fields.reduce((sum, f) => sum + f.size, 0);
  const buf = new ArrayBuffer(nrows * rowSize);
  const view = new DataView(buf);
  for (let r = 0; r < nrows; r++) {
    let offset = r * rowSize;
    for (let c = 0; c < fields.length; c++) {
      ...
      if (field.type === 1) {
        ...
        if (field.size === 1) {
          ...
          if (field.signed) {
            view.setBigInt64(offset, BigInt(Math.round(val)), true);
          } else {
            view.setBigUint64(offset, BigInt(Math.round(val)), true);
          }
        }
      }
      offset += field.size;
  ...
  file.create_compound_dataset({
    name,
    data: buf,
    fieldNames: fields.map((f) => f.name),
    fieldTypes: fields.map((f) => f.type),
    fieldSizes: fields.map((f) => f.size),
    fieldSigns: fields.map((f) => f.signed),
    nrows,
    rowSize
  });
}
...
function writeRois(file, rois, videos, tracks) {
  ...
  roisDs.create_attribute("categories", catStr, null, `S${new TextEncoder().encode(catStr).length}`);
  const namesStr = JSON.stringify(names);
  roisDs.create_attribute("names", namesStr, null, `S${new TextEncoder().encode(namesStr).length}`);
  const srcStr = JSON.stringify(sources);
  roisDs.create_attribute("sources", srcStr, null, `S${new TextEncoder().encode(srcStr).length}`);
...
function writeMasks(file, masks, videos, tracks) {
  ...
  const mCatStr = JSON.stringify(categories);
  masksDs.create_attribute("categories", mCatStr, null, `S${new TextEncoder().encode(mCatStr).length}`);
  const mNamesStr = JSON.stringify(names);
  masksDs.create_attribute("names", mNamesStr, null, `S${new TextEncoder().encode(mNamesStr).length}`);
  const mSrcStr = JSON.stringify(sources);
  masksDs.create_attribute("sources", mSrcStr, null, `S${new TextEncoder().encode(mSrcStr).length}`);
