...
function serializeInstanceGroup(group, session, identities, frameGroup, labeledFrameIndex) {
  ...
  for (const camera of instCameras) {
    ...
    // LUCID local patch (#134): write the inline point dict ONLY as a fallback
    // when no labeled-frame ref (lf_idx, inst_idx) could be resolved. Grouped
    // instances always resolve a ref, so their full 2D pose is no longer ALSO
    // duplicated verbatim into sessions_json — that duplication made the single
    // per-session JSON string blow past V8's max length ("Save failed: invalid
    // string length") on large projects, and roughly doubled .slp size. The ref
    // map round-trips the grouping losslessly (same output the streaming writer
    // already emits). Re-apply after any re-vendor (grep this marker); report
    // upstream to sleap-io. See CLAUDE.md.
    else if (instance) instances[cameraKey] = pointsToDict(instance);
  }
  const payload = {};
